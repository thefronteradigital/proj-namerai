        ...
        <FormInput label="Preferences" className="md:col-span-1">
          <label className="flex items-center justify-between w-full h-9 px-2.5 bg-white border-2 border-slate-300 rounded-lg cursor-pointer hover:border-slate-400 transition-all group box-border shadow-sm">
            ...
            <div className="relative">
              ...
              />
              <div className={`w-8 h-4 rounded-full transition-colors duration-300 ${formState.checkDomains ? 'bg-blue-600' : 'bg-slate-300'}`}></div>
              <div className={`absolute left-0.5 top-0.5 bg-white w-3 h-3 rounded-full shadow-sm transition-transform duration-300 ${formState.checkDomains ? 'translate-x-4' : ''}`}></div>
            </div>
          </label>
        </FormInput>
        </div>
